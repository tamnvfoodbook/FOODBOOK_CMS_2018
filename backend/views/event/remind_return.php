...
$gridColumns = [
    ...
    [
        ...
        'value' => function ($model) {
            $list = '';
            if($model->send_via_sms){
                $list = 'SMS';
            }
            if($model->send_via_zalo){
                if($list){
                    $list = $list.' - ZALO';
                }else{
                    $list = 'ZALO';
    ...
    [
        'label' => 'Số ngày khách chưa trở lại',
        'value' => function ($model) {
            if(@$model->trigger_remind_return->remind_before_day){
                return $model->trigger_remind_return->remind_before_day .' ngày';
            }else{
                return 'Gửi đúng ngày';
            }
