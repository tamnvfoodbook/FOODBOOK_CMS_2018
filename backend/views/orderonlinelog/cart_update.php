...
//echo '<pre>';
//var_dump($model->order_data_item[0]);
//echo '</pre>';
//die();
    ...
    //(function($){
            ...
            $.fn.smartCart = function(options) {
                ...
                return this.each(function() {
                    ...
                    var proContain = $('<div></div>').addClass("col-md-8 productContain");
                    var elmProStyle = $('<div></div>').addClass("box box-success box-solid");
                    var elmProBox = $('<div></div>').addClass("box-header with-border");
                    var elmProTitle = $('<div>Thực đơn - <?= $pos["POS_NAME"] ?></div>').addClass("box-title");
                    ...
                    var elmCartHeaderTitle1 = $('<label >'+labelProducts+'</label>').addClass("col-md-5 no-padding");
                    var elmCartHeaderTitle2 = $('<label>'+labelPrice+'</label>').addClass("col-md-2");
                    var elmCartHeaderTitle3 = $('<label>'+labelQuantityText+'</label>').addClass("col-md-2 no-padding");
                    var elmCartHeaderTitle4 = $('<label >'+labelSubtotal+'</label>').addClass("col-md-2 no-padding scCartTitle4");
                    ...
                    var CartMenu = $('<div></div>').addClass("col-md-4 no-padding");
                    var elmCartStyle = $('<div></div>').addClass("box box-success box-solid");
                    var elmCartBox = $('<div></div>').addClass("box-header with-border");
                    var elmCartTitle = $('<div>Đơn hàng : <?= $model->to_address ?></div>').addClass("box-title addressText");
                    ...
                    var elmDivButton = $('<div></div>').addClass("scCheckoutDiv col-md-12");
                    var btCheckout = $('<input type="submit" value="'+ labelCheckout +'">').addClass("col-md-4 btn btn-success col-md-offset-4");
                    ...
                    elmBottomBar.append(elmBottomSubtotalText).append(elmBottomSubtotalValue).append(elmShowFee).append(elmShowPayment).append(elmDivButton);
                    ...
                    function addCartItemDisplay(objProd,Quantity,newIndex){
                        ...
                        if(pDiscount >0 ){
                            var pTotal = (pPrice - 0) * (Quantity - 0) * (pDiscount - 0);
                        }else{
                        ...
                        var elmCPTitle1 = $('<div></div>').addClass("col-md-5");
                        ...
                        var elmCPTitle2 = $('<span>'+pPrice+'<br/> <span class="priceDown">(&darr;' + pDiscount*100 + ' %)</span></span>').addClass("col-md-2");
...
//                            var pCurI = valueArray[3];
                    ...
                    function updateCartQuantity(idx,qty){
                        ...
                        var newPValue =  prdId + '|' + qty + '|' + curNote + '|' + curI;
                        $(productItem).attr("value",newPValue).attr('selected', true);
                    ...
                    function refreshCartValues(){
                        ...
                        var url = "<?= \yii\helpers\Url::to(['/ajaxapi'])?>" + "&pos_id=" + "<?= $model->pos_id?>" + "&amount=" + subTotal + "&address=" + "<?= $model->to_address?>"  + "&longitude=" + "<?= $model->longitude?>" + "&latitude=" + "<?= $model->latitude?>";
    ...
    <div id="SmartCart" class="scMain clearfix">
        ...
        foreach($itemList->data as $pos){
            echo '<input type="hidden" pimage="'.@$pos->Item_Image_Path_Thumb.'" pprice="'.$pos->Ta_Price.'" pdiscount="'.$pos->Discount_Ta_Price.'" pname="'.$pos->Item_Name.'" pcategory="'.$pos->Item_Type_Name.'" pdesc="'.@$pos->Description.'"  pid="'.$pos->Item_Id.'">';
        }
        ?>
    </div>
