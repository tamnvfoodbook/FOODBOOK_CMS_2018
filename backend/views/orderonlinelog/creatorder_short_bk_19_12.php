...
/*            $('#textPhone').on('paste', function (e) {
            ...
            function addToCart(i,qty,idCombo){
                ...
                if(addProduct.length > 0){
                    ...
                    if(pCombo){
                        addToCartCombo(pId,qty);
                    }else{
                        addToCartNomal(pItemId,qty);
                    }
...
//                    var countProductItem = elmProductSelected.children().length;
                    ...
                    *//*if(productItem.length > 0){
                    ...
                    var prodStr = pId + '|' + qty + '|' + '|' + i+'|' + pDiscount + '|'+ pEatWith + '|'+ '|'+ pPrice ; // 0 Id, 1 số lượng ,2 ghi chú, 3 id in cart, 4 giảm giá, 5 EatWith , 7 Gía
                    var productItem = $('<option></option>').attr("rel",newIndex).attr("value",prodStr).attr("combo",idCombo).attr('selected', true).html(pName);


                    if(!pCombo){  // Nếu không phải là món combo chính thì mới add vào list
            ...
            function addToCartNomal(itemId,qty){
            ...
            function addToCartCombo(itemId,qty,idCombo){
...
//                    var pIsnomalCombo = $(addProduct).attr(attrIsNomalCombo);
                    ...
                    var prodStr = pId + '|' + qty + '|' + '|' + i+'|' + '|'+ pEatWith + '|'+ '|'+ pPrice ;  // 0 Id, 1 số lượng ,2 ghi chú, 3 id in cart, 4 giảm giá, 5 EatWith , 6 Gía, 7 Dis count
                    var productItem = $('<option></option>').attr("rel",newIndex).attr("value",prodStr).attr("combo",idCombo).attr('selected', true).html(pName);
...
//                            var pCurI = valueArray[3];
                ...
                var pCombo = $(productItem).attr("combo");
                if(pCombo != undefined){
                    var productItemCombo = elmProductSelected.children("option[combo=" + pCombo + "]");
                    var pValueCombo = $(productItemCombo).attr("rel");
                    removeFromCart(pValueCombo);
                }
...
//                console.log(productItem);
            ...
            function popupCombo(idx,combo,comboItemId){
...
//                console.log('value.item_id_list',value.item_id_list);
                ...
                $.each(combo, function( key, value ){
                    ...
                    if(value.item_id_list) {
                        ...
                        console.log(dataItem[value.item_id]);

                        var inputQty = $('<select></select>').attr("id","comboQty"+itemComboDetail[5]).attr("name","comboQty"+itemComboDetail[5]).addClass("form-control");
                        for (i=1;i<= value.quantity ;i++){
                            if(i ==  value.quantity ){
                                inputQty.append($('<option selected></option>').val(i).html(i));
                            }else{
                        ...
                        eatWithelm = $('<div class="funkyradio-primary">' +
                        '<input type="checkbox" name="comboCheckbox" id="'+tmpGroup+value.item_id+'"  value="'+itemComboDetail[5]+'" comboid="'+itemComboDetail[5]+'" itemid ="'+value.item_id+'"  checked/>' +
                        '<label for="'+tmpGroup+value.item_id+'">'+ itemComboDetail[1] +'</label> ' +
                        '</div>');
...
//                var pDiscount = $(pObj).attr(attrProductDiscount);
                ...
                var newPValue =  prdId + '|' + curQty + '|' + curNote + '|' + curI+ '|' + cdisCount + '|' + cEatWith + '|' + sledEatWith + '|'+  pPrice;
...
//                                    console.log('#eatWith'+pIdx);
            ...
            function updateCartNote(idx,note){
                ...
                var newPValue =  prdId + '|' + curQty + '|' + note + '|' + curI+ '|' + cdisCount+ '|' + cEatWith+ '|' + csledEatWith + '|' + pPrice;
                $(productItem).attr("value",newPValue).attr('selected', true);
...
<div id="calContent">
    <div class="form-group">
        ...
        <input name="txt-vat" class="form-control" id="input-vat">


        <label for="email">Phí dịch vụ</label>
        <input name="txt-service_charge" class="form-control" id="input-charge">
...
//                    $("#booking_order").removeClass("fb-grid-loading");
    ...
    $(function(){
        $('#btn_update_option').click(function(){
            ...
            var totalText = $(".scLabelSubtotalValue").html();
            var total = totalText.replace(",","");
            ...
            var newTotal = parseInt(total) + parseInt(total*vat/100) + parseInt(charge) - discountBill ;
