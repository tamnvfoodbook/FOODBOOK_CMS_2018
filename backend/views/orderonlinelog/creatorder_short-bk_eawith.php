...
//    var item_id_list = $( comboInput ).attr("comboitems");
    ...
    var comboGroupHeader = $('<div>Số lượng yêu cầu : '+ combo_qty +'</div>').addClass("combo-group-header");
...
//            console.log('dataItem[itemId]',dataItem[itemId]);
            var inputQty = $('<select></select>').attr("id", "comboQty" + itemId ).attr("name", "comboQty" + itemId).attr("price", detailItem[4] ).addClass("form-control sltpcombo");
...
//                var ots_price = element.attr("ots_price");
                ...
                groupCombo.push(combo_qty);
                groupCombo.push(itemId+'*'+qTycb+'*'+ots_price);
        ...
        if(checkFail){
            ...
            var contentEatwith = $('#content-eatwith'+parent_id);
            console.log('content-eatwith',parent_id);
            console.log('contentEatwith',contentEatwith);
...
//                console.log(arrSelected);
                ...
                $.each(listItem, function( key, value ){
                    ...
                    if(jQuery.inArray(value.Item_Id, arrEatWith) != -1) {
                        ...
                        console.log($('#categoryEatWith'+value.Item_Type_Id).length);
                        console.log('value.Item_Type_Id',value.Item_Type_Id);
                        console.log('checkboxEatWith',checkboxEatWith);
                        ...
                        if($('#categoryEatWith'+value.Item_Type_Id).length)         // use this if you are using id to check
                        {
                            ...
                            elmCategory = $('<div>'+value.Item_Type_Id+'</div>').attr('id','categoryEatWith'+value.Item_Type_Id).addClass('category');
...
//                        console.log('value',value);
                        ...
                        if(value.combo_details != undefined){
                            var eatWithelm = $('<input type="checkbox" name="eatWith" class="eatWithCkb" onchange="popupComboEatwith('+value.Item_Id+')" comboitems='+value.combo_details+' comboqty="'+value.quantity+'" parentid="'+ idx +'" id="'+value.Item_Id+'"  value="'+value.Item_Id+'_*_'+ value.Ta_Price+'_*_' +value.Item_Name+'" '+checkedItem+'/>' +
                            '<label for="'+value.Item_Id+'">'+ value.Item_Name +'</label>');

                        }else{
                            eatWithelm = $('<input type="checkbox" name="eatWith" class="eatWithCkb" id="'+value.Item_Id+'"  value="'+value.Item_Id+'_*_'+ value.Ta_Price+'_*_' +value.Item_Name+'" '+checkedItem+'/>' +
                            '<label for="'+value.Item_Id+'">'+ value.Item_Name +'</label>');
                        }
