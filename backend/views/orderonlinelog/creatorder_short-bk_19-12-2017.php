...
//                console.log('value.item_id_list',value.item_id_list);
                ...
                $.each(combo, function( key, value ){
                    ...
                    if(value.item_id_list) {
                        ...
                        var inputQty = $('<select></select>').attr("id","comboQty"+itemComboDetail[0]).attr("name","comboQty"+itemComboDetail[0]).addClass("form-control");
                        for (i=1;i<= value.quantity ;i++){
                            if(i ==  value.quantity ){
                                inputQty.append($('<option selected></option>').val(i).html(i));
                            }else{
                        ...
                        eatWithelm = $('<div class="funkyradio-primary">' +
                        '<input type="checkbox" name="comboCheckbox" id="'+tmpGroup+value.item_id+'"  value="'+itemComboDetail[0]+'" comboid="'+itemComboDetail[0]+'" checked/>' +
...
//                        var idCombo = this.value;
                        ...
                        if( groupCombo[groupId] === undefined ) {
                            ...
                            groupCombo[groupId].push(idCombo+'*'+qTycb);
                        }else{
                            groupCombo[groupId].push(idCombo+'*'+qTycb);
                        }
                    ...
                    if(checkFail){
                        ...
                        if(comboItemId){
                            ...
                            console.log('groupCombo',groupCombo);
                                        ...
                                        $.each( value, function( keyCheckcombo, valueCheckcombo ) {
                                            ...
                                            if(value  !== undefined){
                                                if(keyCheckcombo !== 0 && keyCheckcombo !== 1){
                                                    ...
                                                    addToCart(valueArray[0],valueArray[1],idgroupCb);
                                                }
                                            }
                                        });
                                    }
            ...
            function fnOnlySumEatWith(sledEatWith,curQty,cdisCount,pIdx){
                ...
                if(typeof sledEatWith != 'undefined' && sledEatWith != 'undefined' && sledEatWith != ''){
                    ...
                    $.each(arrEatWith, function( key, value ){
                        ...
                        console.log('sledEatWith',sledEatWith);
                        console.log('arrOneEatWith',arrOneEatWith);
    ...
    if($listItems){
        foreach($listItems as $pos){
            ...
            if(!$pos->Is_Eat_With){
                ...
                echo '<input type="hidden" pimage="'.$pos->Item_Image_Path_Thumb.'" pfullname ="'.@$pos->Item_Name.'"  pprice="'.$pos->Ta_Price.'" pdiscount="'.$mDiscount.'" piseatwith="'.@$pos->Is_Eat_With.'" peatwith="'.@$pos->Item_Id_Eat_With.'"  pname="'.$itemName.'" pcategory="'.@$itemTypeMap[$pos->Item_Type_Id].'" pdesc="'.@$pos->Description.'" pid="'.$pos->Id.'" pcomboid="'.@$pos->Combo_Item_Id.'" pisnomalcb="'.@$pos->Is_Nomal_Combo.'" pcomboex='.@$pos->combo_details.'>';
