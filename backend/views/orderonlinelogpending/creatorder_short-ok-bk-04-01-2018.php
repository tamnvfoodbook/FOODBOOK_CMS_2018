...
var ItemNotExit = '<?= $listNotExit?>';
if(ItemNotExit != ''){
    alert('Món nhà hàng không có: '+ ItemNotExit);
}
...
$(document).ready(function(){
    ...
    $.fn.smartCart = function(options) {
        ...
        return this.each(function() {
            ...
            elmTotal.append(elmBottomSubtotalText).append(elmBottomSubtotalValue);

            var elmVoucher = $('<div></div>');
            var elmVoucherLabel = $('<label>'+labelVoucher+':</label>').addClass("col-md-5");
            var elmVoucherCodeLabel = $('<label>'+labelVoucherCode+'</label>').addClass("col-md-3 vouchercode");
            var elmVoucherValue = $('<span>'+getMoneyFormatted(<?= $voucherDiscount ?>)+'</span>').addClass("scLabelSubtotalValue col-md-2 no-padding codeValue");
            var elmVoucherCheckCode = $('<div><button class="btn btn-primary btn-sm" id="btn_checkvoucher" type="button"><span class="glyphicon glyphicon-tag"></span></button></div>').addClass("col-md-1");
            elmVoucher.append(elmVoucherLabel).append(elmVoucherCodeLabel).append(elmVoucherValue).append(elmVoucherCheckCode);
            elmBottomBar.append(elmTotal).append(elmVoucher);
...
//            var elmInfo = $('<div></div>').addClass("scDetailOrder");
            ...
            var elmBottomPhoneValue = $('<input name="textPhone" value="<?= $phoneNumber ?>" id="textPhone">').addClass(" col-md-5 controls_detail required");
            ...
            elmShowPayment.append(elmBottomPhoneText).append(elmBottomPhoneValue).append(elmHistoryBtn);

            // Show Info Name
            var elmBottomInfoText = $('<label>'+labelInfo+':</label>').addClass("scLabelSubtotalText col-md-5 ");
            var elmBottomInfoValue = $('<input name="textName" value="<?= @$model->username ?>" id="textName">').addClass(" col-md-5 no-padding controls_detail");
            ...
            elmShowPayment.append(elmBottomInfoText).append(elmBottomInfoValue).append(elmProfileEditBtn);

            // Show address Customer
            var elmBottomAddressText = $('<label>'+labelAddres+':</label>').addClass("scLabelSubtotalText col-md-5");
            var elmBottomAddressValue = $('<input name="addressTxt" value="<?= @$model->to_address ?>"  id="addressTxt">').addClass(" col-md-5 no-padding controls_detail");
            var elmAdreeEditBtn = $('<div><a href="#myMapModal" class="btn btn-primary btn-sm" data-toggle="modal" ><span class="glyphicon glyphicon-map-marker"></span></a></div>').addClass("col-md-1");
            ...
            if(!isBookking){
                elmShowPayment.append(elmBottomAddressText).append(elmBottomAddressValue).append(elmAdreeEditBtn);
            }else{
            ...
            var elmBottomNoteValue = $('<input name="noteTxt" value="'+noteValue+'" >').addClass("col-md-6 no-padding controls_detail");
            ...
            var elmFoodbookVoucherCode = $('<input name="coupon_log_id" value="<?= @$model->coupon_log_id ?>" type="hidden">');
            var elmCreatby = $('<input name="created_by" value="<?= @$model->created_by ?>" type="hidden">');
            elmShowPayment.append(elmBottomNoteText).append(elmBottomNoteValue).append(elmIdpending).append(elmFoodbookCode).append(elmFoodbookVoucherCode).append(elmCreatby);
...
//            alert(posSelect);
            ...
            elmBottomBar.append(elmShowPayment).append(elmDivButton);
            ...
            if(elmProductSelected.length <= 0){
                ...
                if(checkTheFirt == 0){
                    ...
                    $.each(objSelect, function( key, value ){
                        var prodStrTmp = value.Id + '|' + value.Quantity + '|' + value.Note +  '|' + value.Id +'|' + '|' + '|' + value.Eat_with; // 0 Id, 1 số lượng ,2 ghi chú, 3 id in cart, 4 giảm giá, 5 EatWith, 6 Eatwith Selected
                        var productItemTmp = $('<option></option>').attr("rel",value.Id).attr("value",prodStrTmp).attr('selected', true).html(value.Item_Name);
                        elmProductSelected.append(productItemTmp);
//                        var pObj = products.eq(key);
                        var pObj = products.filter("[pid='"+value.Id+"']");
                        addCartItemDisplay(pObj,value.Quantity,value.Id);
                    });
                    checkTheFirt = 1;
                }
                refreshCartValues();
            ...
            function addToCart(i,qty){
                var addProduct = products.filter("[pid='"+i+"']");
//                console.log(i);
//                console.log(addProduct);
//                console.log(products);
//                alert(addProduct.length);
...
//                var pObj = products.eq(curI);
                var pObj = products.filter("[pid='"+curI+"']");
...
//                var pObj = products.eq(curI);
            ...
            function fnSumEatWith(sledEatWith,curQty,cdisCount,pIdx){
                ...
                if(typeof sledEatWith != 'undefined' && sledEatWith != 'undefined' && sledEatWith != ''){
                    ...
                    var eatWithContent = $('<div></div>').addClass("col-md-12");
                    $.each(arrEatWith, function( key, value ){
                        ...
                        var eatWithElment = $('<div class="row"></div>');
                        var ewPrice = $('<div>'+arrOneEatWith[1]+'</div>').addClass("col-md-3 no-padding");
...
//                var pObj = products.eq(curI);
            ...
            function refreshCartValues(){
                ...
                elmProductSelected.children("option").each(function(n) {
                    ...
                    fnSumEatWith(pEatWithSelected,pQty,pDiscount,pIdx);

//                    console.log('pEatWith',pEatWithSelected);
...
//                    var pObj = products.eq(pI);
                    var pObj = products.filter("[pid='"+pI+"']");

//                    var newIndex = idGen.getId();
//                    addCartItemDisplay(pObj,pQty,pI);
                    ...
                    sTotal = (((pPrice - 0) * (pQty - 0)) - ((pPrice - 0) * (pQty - 0) * (pDiscount/100))) + sumEatWith;
                    $('#lblTotal'+pIdx).html(getMoneyFormatted(sTotal));
            ...
            function populateProducts(searchString){
                ...
                $(products).each(function(i,n){
                    ...
                    if(isValid && isCategoryValid) {
                        ...
                        var btAddToCart = $('<a></a>').attr("href","#").attr("rel",productId).attr("title",labelAddToCartButton).addClass("");
