...
/*            echo 'Chọn Top cuối:';
    ...
    <div class="col-md-3 col-md-offset-9">
        <div class="input-group">
            ...
            <input type="text" class="form-control pull-right" id="reservation" readonly="readonly" value="<?= date('d/m/Y', strtotime('-30 days')).' - '.date('d/m/Y') ?>"/>
        </div><!-- /.input group -->
    </div>
</div>
<br>
...
//            ]
    ...
    $(document).ready(function() {
        $.fn.ajaxData = function() {
            ...
            $.ajax({type: "POST",
                url: "<?= Url::toRoute('/orderrate/report')?>",
                data: {dateRanger: $("#reservation").val()},

                beforeSend: function() {
                    //that.$element is a variable that stores the element the plugin was called on
                    $("#orderrate-index").addClass("fb-grid-loading");
                },
                complete: function() {
                    //$("#modalButton").removeClass("loading");
                    $("#orderrate-index").removeClass("fb-grid-loading");
                },

                success:function(result){
                    $("#orderrate-index").html(result);
                }
            });
        }
    });
