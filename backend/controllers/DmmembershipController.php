...
class DmmembershipController extends Controller
{
    ...
    public function actionReport()
    {
        ...
        $dataProvider = $eventModelSeach->searchByApi(Yii::$app->request->queryParams);
...
//        $paramData['data']['expected_approach'] = count($dataProvider->allModels);
        $paramData['expected_approach'] = count($dataProvider->allModels);
...
//            $preData = json_decode($post['predata'],true);
            ...
            if($model->MEMBER_TYPE){
                $preData['membership_type'] = $model->MEMBER_TYPE;
            }

            $filterData = array_filter($preData, create_function('$value', 'return $value !== "" && $value !== NULL;'));

            $apiName = 'ipcc/create_event';
            $apiPath = Yii::$app->params['CMS_API_PATH_IPOS'];

            $data = ApiController::getApiByMethod($apiName,$apiPath,$filterData,'POST');
