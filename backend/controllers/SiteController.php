...
class SiteController extends Controller
{
    ...
    static function actionCountDayMongoDate($start,$end,$format = 'Y-m-d',$step = '+1 day'){
...
//        'rateArrayByReson'=> $rateArrayByReson,
    ...
    public function dataWeek($start,$end){

        $apiPath = Yii::$app->params['CMS_API_PATH_IPOS'];
        $name = 'ipcc/total_extra_info';
        $param = array(
            'date_start' => $start,
            'date_end' => $end,
        );
        ...
        $dayArray = SiteController::actionCountDay($start,$end,'Y-m-d H:i:s');
        $dataWeek = array();
        $TA_count = 0;
        $OTS_count = 0;

        foreach($dayArray as $date){
            foreach((array)@$total_data->data->List_Bill_Revenue_By_Date as $value){
                if($date == $value->Sale_Date){
                    $dataWeek['TA_Count'][$date] = ['value' => $value->TA_Count];
                    $dataWeek['OTS_Count'][$date] = ['value' => $value->OTS_Count];
                    $TA_count =  $TA_count + $value->TA_Count;
                    $OTS_count = $OTS_count + $value->OTS_Count;
                    break;
                }else{
                    $dataWeek['TA_Count'][$date] = ['value' => 0];
                    $dataWeek['OTS_Count'][$date] = ['value' => 0];
                }
            }


            foreach((array)@$total_data->data->List_Rate_Revenue_By_Date as $rate){
                $rateDate = $rate->Year.'-'.$rate->Month.'-'.$rate->Day;
                if($date == date('Y-m-d 00:00:00',strtotime($rateDate))){
                    $dataWeek['rate'][$date] = ['value' => number_format($rate->Rate_Count,1)];
                    break;
                }else{
                    $dataWeek['rate'][$date] = ['value' => 0 ];
                }
            }
        }

        /*echo '<pre>';
        var_dump($total_data->data->List_Rate_Revenue_By_Date);
        var_dump($dataWeek['rate']);
        echo '</pre>';
        die();*/
...
/////////////////////////////
    public function sumPosSstatis($posId,$start,$end){
        ...
        $dateStartWeek = date('Y-m-d 00:00:00',(strtotime($DAY2)));
        $todayOfWeek = date('Y-m-d H:i:s');

        $dataOnWeek = self::dataWeek($dateStartWeek,$todayOfWeek);

        $dayArray = SiteController::actionCountDay($dateStartWeek,$todayOfWeek,'D');
...
//        die();
    ...
    public function actionStatistics($dateRanger = null){
        ...
        if($dateRanger){
            ...
            $date_start = date("Y-m-d 00:00:00", strtotime($date_start_tmp));


            $date_end_tmp = str_replace('/', '-', $arrTime[1]);
            $date_end = date("Y-m-d 23:59:59", strtotime($date_end_tmp));
...
//        die();
    ...
    /*public function actionIndex(){
        return $this->redirect(['/orderonlinelog'],302);
    }*/
